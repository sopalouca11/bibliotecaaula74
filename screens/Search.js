...
// import { View, Text, StyleSheet, SnapshotViewIOSComponent, FlatList } from "react-native";
...
//             onEndReached={() => fetchMoreTransactions(searchText)}
...
export default class SearchScreen extends Component {
  ...
  fetchMoreTransactions = async text => {
    ...
    const { lastVisableTransaction, allTransactions } = this.state;
    if (enteredText[0] === "B") {
      const query = await db

        .collection("transactions")
        .where("bookId", "==", text)
        .startAfter(lastVisableTransaction)
        .limit(10)
        .get();
      query.docs.map(doc => {
        this.setState({
